...
USE `equipe385145`$$
DROP TRIGGER IF EXISTS `aluno_BEFORE_INSERT` $$
SHOW WARNINGS$$
USE `equipe385145`$$
CREATE DEFINER = CURRENT_USER TRIGGER equipe385145.aluno_BEFORE_INSERT 
...
BEGIN
    ...
    IF NEW.data_de_conclusao_prevista < now() THEN
        ...
        DELETE usuario, aluno 
		FROM usuario LEFT OUTER JOIN aluno
		ON aluno.matricula = usuario.matricula 
		WHERE tipo = 'aluno' and cod_curso is null;
		
        SET msg = concat('O aluno ja esta formado');
